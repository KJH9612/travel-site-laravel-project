		...
		<link rel="stylesheet" href="/h_css/jquery.timepicker.css">				
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
						<form method="post" action="{{route('a_hreservation.update', $row->id)}}">
							@method('PATCH')
							@csrf
                            ...
                            <table class="table table-bordered table-striped">
								  <tbody>
									...
									<tr>
									  ...
									  <td width="80%">
										<input type="text" class="form-control" name="name" value="{{ $row->consumer_name }}" readonly>
									  </td>
									</tr>
									<tr>
									  <th scope="row" width="20%" style="vertical-align:middle;">Check_in</th>
									  <td width="80%">
									  	<input type="text" name="check_in" id="check_in" class="form-control checkin_date" value="{{ $row->check_in }}" data-target="#check_in" placeholder="입실일 선택" onchange='fn_check()'>
									...
									<tr>
									  ...
									  <td width="80%">
										<input type="text" name="check_out" id="check_out" class="form-control checkin_date" value="{{ $row->check_out }}" placeholder="퇴실일 선택" onchange='fn_check()'>
									...
									<tr>
									  ...
									  <td width="80%">
										<input type="text" class="form-control" name="adult" id="adult" value="{{ $row->adult }}" onkeyup='fn_check()'>
									...
									<tr>
									  ...
									  <td width="80%">
										<input type="text" class="form-control" name="child" id="child" value="{{ $row->child }}" onkeyup='fn_check()'>
									...
									<tr>
									  ...
									  <td width="80%">
										<input type="text" class="form-control" name="hotel" value="{{ $row->hotel }}" readonly>
										@error('hotel') {{ $message }} @enderror
									  </td>									 
									</tr>
									<tr>
									  ...
									  <td width="80%">
									  	<select name="hroom_id" id="type" class="form-control" onchange='fn_check()'>
											...
											@foreach($list as $row1)
												@if($row->hotel_id == $row1->hotel_id)
													@if($row->hroom_id == $row1->id)
														<option value="{{$row1->id}}" selected>{{$row1->type}}</option>				
													@else
														<option value="{{$row1->id}}">{{$row1->type}}</option>						
													@endif
												@endif
											@endforeach											
									   </select>
									   ...
									   @foreach($list as $row1)
											@if($row->hotel_id == $row1->hotel_id)
												<input type="text" class="form-control" name="rprice{{$row1->id}}" id="rprice{{$row1->id}}" value="{{ $row1->price }}" style="display:none;">
											@endif
										@endforeach
									  </td>									 
									</tr>									
									<tr>
									  ...
									  <td width="80%">
										<input type="text" class="form-control" name="price" id="price" value="{{number_format($row->price)}}원" readonly>
										<input type="text" class="form-control" name="realprice" id="realprice" value="{{$row->price}}" style="display:none;">
									  </td>									 
									</tr>
									<tr>
										...
										<div class="row">				
											...
											<div class="col-1" style="display: flex; align-items: center; justify-content: left;">
												<div class="form-field" >
												  @if($row->breakfast == 'on') 
													<input type="checkbox" name="breakfast" id="breakfast" data-toggle="switchbutton" data-onstyle="primary" data-offstyle="outline-light" onchange="javascript:check1();" checked>
												  @else
											...
											<div class="col-1" style="display: flex; align-items: center; justify-content: start;">
												<div class="form-field" >
												  @if($row->bedsize == 'on') 
													<input type="checkbox" name="bedsize" id="bedsize" data-toggle="switchbutton" data-onstyle="primary" data-offstyle="outline-light"  onchange="javascript:check2();" checked>
												  @else
											...
											<div class="col-1" style="display: flex; align-items: center; justify-content: start;">
												<div class="form-field" >
												  @if($row->wifiegg == 'on') 
													<input type="checkbox" name="wifiegg" id="wifiegg" data-toggle="switchbutton" data-onstyle="primary" data-offstyle="outline-light" onchange="javascript:check3();" checked>
												  @else 
